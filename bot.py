...
SPOTIPY_CLIENT_ID = "bcb2f4afb3bb42468e558514105ca513"
SPOTIPY_CLIENT_SECRET = "3247fc672e574cd7beb50fc84493d365"
GENIUS_ACCESS_TOKEN = "UB0r_tjB5iyT91JHWN39udoinuMB-MM6hD1iVwS1QTFTqjdXoLj1CaWelPgw5I3w"
...
# Queue de musique et historique
music_queue = deque()
previous_tracks = deque()
loop_mode = False
loop_queue_mode = False
...
def get_lyrics(song_name, artist_name=""):
    ...
    if response.status_code == 200:
        ...
        if hits:
            ...
            lyrics_div = soup.find("div", class_="lyrics") or soup.find_all("div", class_="Lyrics__Container")
            if lyrics_div:
...
class MusicControls(discord.ui.View):
    ...
    async def previous(self, interaction: discord.Interaction, button: discord.ui.Button):
        await previous_track(self.ctx)
        await interaction.response.defer()

    @discord.ui.button(label="⏸️ / ▶️", style=discord.ButtonStyle.grey)
...
async def play(ctx, *, song_name: str):
    try:
        if not ctx.author.voice:
            embed = discord.Embed(title="❌ Erreur", description="Tu dois être dans un salon vocal !", color=discord.Color.red())
            await ctx.send(embed=embed)
            return
...
async def play_next(ctx, voice_client):
    if music_queue:
        ...
        embed = discord.Embed(title="🎵 Lecture en cours", description=f"**{song_name}**", color=discord.Color.green())
        embed.add_field(name="📜 Paroles", value=lyrics, inline=False)
        embed.set_footer(text=f"Durée : {duration_str}")
        embed.set_thumbnail(url=thumbnail_url)
        view = MusicControls(ctx)
        await send_embed(ctx, embed, view)
    else:
        embed = discord.Embed(title="📭 Fin de la queue", description="Plus aucune musique à jouer.", color=discord.Color.orange())
...
async def previous_track(ctx):
    if previous_tracks:
        last_song = previous_tracks.popleft()
        music_queue.appendleft(last_song)
        if ctx.voice_client and ctx.voice_client.is_playing():
            ctx.voice_client.stop()
        await play_next(ctx, ctx.voice_client)
    else:
        await ctx.send("❌ Aucune musique précédente !")
